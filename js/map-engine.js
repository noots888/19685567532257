...
/* myMap v3.1.239: retry asset-bay crossing marker draw after direct asset search pan/zoom settles. */
(function(){
  const ME=typeof window!=='undefined'?window.MapEngine:null;
  if(!ME||ME.__v239AssetBayCrossingRetry)return; ME.__v239AssetBayCrossingRetry=true;
  const oldShowAsset=ME.showAsset;
  if(typeof oldShowAsset==='function'){
    ME.showAsset=function(asset,zoom){
      const res=oldShowAsset.apply(this,arguments);
      if(asset&&Number.isFinite(Number(asset.lat))&&Number.isFinite(Number(asset.lon))){
        [320,850,1400].forEach(ms=>setTimeout(()=>{try{window.HVCrossingsLayer?.showBayForAsset?.(asset,{silent:true});}catch(_){ }},ms));
      }
      return res;
    };
  }
...
/* myMap v3.1.240: patrol panel only uses transmission circuit assets.
   ESA overlay records, utility sidecars and distribution assets are kept off the GPS nearest/next/circuit readout. */
(function(){
  const ME=typeof window!=='undefined'?window.MapEngine:null;
  if(!ME||ME.__v240PatrolTxOnly)return; ME.__v240PatrolTxOnly=true;
  const clean=(v)=>String(v??'').trim();
  const upper=(v)=>clean(v).toUpperCase();
  const compact=(v)=>{try{return window.SearchEngine?.compact?SearchEngine.compact(v):upper(v).replace(/[^A-Z0-9]+/g,'');}catch(_){return upper(v).replace(/[^A-Z0-9]+/g,'');}};
  const distKinds=new Set(['dx-pole','distribution-pole','dist-pole','transformer','distribution-transformer','streetlight','street-light','luminaire','pillar','electrical-pillar','enclosure','electrical-enclosure','service-pit','lv-cable','hv-cable','conductor-span','utility-draw-context','esa-overlay','hv-crossing','note']);
  function rawOf(a){return (a&&typeof a==='object')?(a.raw||{}):{};}
  function assetIdForPatrol(a){const raw=rawOf(a); return [a?.id,a?.assetId,a?.globalId,a?.sourceFile,raw.OBJECTID,raw.objectid,raw.GID,raw.gid,a?.structureId,a?.structure,a?.poleNumber,a?.lat,a?.lon,a?.label].map(clean).join('|');}
  function hasEsaGeom(a){const r=rawOf(a); return !!clean(r.ESA_GEOM_POLY||r.ESA_GEOM||r.esa_geom_poly||r.esa_geom);}
  function looksDistribution(a){
    const r=rawOf(a);
    const kind=clean(a?.kind||r.KIND||r.kind||r.asset_type||r.ASSET_TYPE).toLowerCase();
    if(distKinds.has(kind)||/^utility-/i.test(kind))return true;
    const text=upper([a?.kind,a?.category,a?.label,a?.sourceFile,a?.sourcePath,a?.line,r.ASSET_TYPE,r.asset_type,r.ASSET_CLASS,r.asset_class,r.CATEGORY,r.category,r.LAYER,r.layer,r.SOURCE_LAYER,r.source_layer,r.FIELD_MAP_LAYER,r.field_map_layer,r.NETWORK_TYPE,r.network_type,r.DESCRIPTION,r.description,r.EQUIP_GRP_ID_DESC,r.equip_grp_id_desc].join(' '));
    if(/ESA[_\s-]*OVERLAY|ENVIRONMENTALLY\s*SENSITIVE|ESA_GEOM|UTILITY[_\s-]*(DRAW|ROUTE|CONTEXT)|DRAW_CONTEXT_SIDECAR/.test(text))return true;
    if(/DISTRIBUTION[_\s-]*(POLE|ASSET|NETWORK|TRANSFORMER|UNDERGROUND|CABLE)|\bDX[_\s-]*POLE\b|\bDIST\s+POLE\b|TRANSFORMER|KVA|STREET\s*LIGHT|STREETLIGHT|LUMINAIRE|PILLAR|SERVICE\s*PIT|LOW\s*VOLTAGE|\bLV\b|SERVICE\s*CABLE|ELECTRICAL\s+ENCLOSURE/.test(text))return true;
    return false;
  }
  function isBlockedPatrolAsset(a){
    if(!a||typeof a!=='object')return true;
    if(!Number.isFinite(Number(a.lat))||!Number.isFinite(Number(a.lon)))return true;
    if(hasEsaGeom(a)||looksDistribution(a))return true;
    try{if(window.UtilitiesEngine?.isUtility?.(a))return true;}catch(_){ }
    try{if(window.SearchEngine?.isUtilityAsset?.(a)||window.SearchEngine?.isHVCrossingAsset?.(a)||window.SearchEngine?.isConductorSpanAsset?.(a))return true;}catch(_){ }
    try{if(window.MapEngine?.isContextOnlyAsset?.(a))return true;}catch(_){ }
    return false;
  }
  function displayLine(line){try{return window.SearchEngine?.formatCircuitName?.(line)||line||'';}catch(_){return line||'';}}
  function refsFor(a){
    ...
    try{refs=window.SearchEngine?.lineRefsForAsset?.(a,true)||[];}catch(_){refs=[];}
    if(!refs.length&&a?.line)refs=[{line:a.line}];
    return refs.map(r=>displayLine(r?.line||r||'')).filter(Boolean);
  }
  function isDisplayLine(line){try{return !!window.SearchEngine?.isDisplayableTransmissionCircuitLine?.(line);}catch(_){return /[A-Z].*[-/].*\d|\d.*[-/].*[A-Z]/i.test(String(line||''));}}
  function patrolLineForAsset(a){
    if(isBlockedPatrolAsset(a))return '';
    try{if(window.SearchEngine?.canContributeTransmissionCircuit&&!SearchEngine.canContributeTransmissionCircuit(a))return '';}catch(_){ }
    const lines=refsFor(a).filter(isDisplayLine);
    return lines[0]||'';
  }
  function isPatrolTxAsset(a){return !!patrolLineForAsset(a);}
  function filterPatrolAssets(list){
    const out=[], seen=new Set();
    for(const a of (Array.isArray(list)?list:[])){
      if(!isPatrolTxAsset(a))continue;
      const id=assetIdForPatrol(a); if(seen.has(id))continue; seen.add(id); out.push(a);
    }
    return out;
  }
  ME.isPatrolTransmissionAsset=isPatrolTxAsset;
  ME.patrolCircuitLineForAsset=patrolLineForAsset;
  ME.filterPatrolTransmissionAssets=filterPatrolAssets;

  const oldCircuitFor=ME.circuitForGpsAsset;
  ME.circuitForGpsAsset=function(a){
    const line=patrolLineForAsset(a);
    if(line)return line;
    return a?'' :(oldCircuitFor?oldCircuitFor.call(this,a):this.currentCircuit||'');
  };

  ME.buildPatrolCircuitCandidates=function(){
    const assets=Array.isArray(window.App?.assets)?App.assets:[];
    const stamp=[assets.length,window.SearchEngine?.lineMap?.size||0,window.SearchEngine?.indexedAssetIds?.size||0,'v240'].join('|');
    if(this._patrolCircuitCandidateStamp===stamp&&Array.isArray(this._patrolCircuitCandidates))return this._patrolCircuitCandidates;
    const out=[]; const seen=new Set();
    for(let i=0;i<assets.length;i++){
      const a=assets[i];
      const line=patrolLineForAsset(a);
      if(!line)continue;
      const k=[compact(line),assetIdForPatrol(a),i].join('|'); if(seen.has(k))continue; seen.add(k);
      out.push({lat:Number(a.lat),lon:Number(a.lon),line,asset:a});
    }
    this._patrolCircuitCandidates=out;
    this._patrolCircuitCandidateStamp=stamp;
    return out;
  ...
  function currentCircuitPool(ctx){
    const line=displayLine(ctx.currentCircuit||ctx.lastFullCircuitLabel||'');
    const key=compact(line.replace(/^circuit\s+/i,''));
    if(!key)return [];
    try{
      const group=window.SearchEngine?.lineMap?.get?.(key);
      return filterPatrolAssets(group?.assets||[]);
    }catch(_){return [];}
  }
  ME._gpsActivePool=function(){
    const full=filterPatrolAssets(Array.isArray(this.lastFullCircuitAssets)?this.lastFullCircuitAssets:[]);
    if(full.length)return full;
    const drawn=filterPatrolAssets(Array.isArray(this.lastDrawnAssets)?this.lastDrawnAssets:[]);
    if(drawn.length)return drawn;
    const circuit=currentCircuitPool(this);
    if(circuit.length)return circuit;
    return [];
  };
  ...
  ME.nearbyAssetsForGps=function(lat,lon){
    const active=this._gpsActivePool?.()||[];
    if(active.length){
      const quick=this._gpsNearbyFromPool?.(active,lat,lon,260)||active;
      return filterPatrolAssets(quick.length?quick:active);
    }
    let old=[];
    try{old=oldNearby?oldNearby.call(this,lat,lon):[];}catch(_){old=[];}
    old=filterPatrolAssets(old);
    if(old.length)return old;
    const all=(this.buildPatrolCircuitCandidates?.()||[]).map(c=>c.asset).filter(Boolean);
    const quick=this._gpsNearbyFromPool?.(all,lat,lon,260)||all;
    return filterPatrolAssets(quick);
  };

  function sameAsset(a,b){return a===b||assetIdForPatrol(a)===assetIdForPatrol(b);}
  function nearestFrom(ctx,origin,list,skip=null){
    let best=null,bestM=Infinity;
    for(const a of filterPatrolAssets(list)){
      if(skip&&sameAsset(a,skip))continue;
      const m=ctx.distanceM(origin,a);
      if(Number.isFinite(m)&&m<bestM){best=a;bestM=m;}
    }
    return best?{asset:best,m:bestM}:null;
  }
  ME.gpsNearestSummary=function(){
    const g=this.gpsLast; if(!g)return null;
    const now=Date.now();
    const cacheKey=[Math.round(Number(g.lat)*10000),Math.round(Number(g.lon)*10000),Math.round(Number(g.heading)||0),this._patrolCircuitCandidateStamp||'',this.currentCircuit||'',this.lastFullCircuitLabel||'',this.currentDisplay||'','v240'].join('|');
    if(this.gpsNearestCache&&this.gpsNearestCache.cacheKey===cacheKey&&now-(this.gpsNearestCache.ts||0)<(this.gpsProfile==='helicopter'?900:1600))return this.gpsNearestCache;
    const origin={lat:Number(g.lat),lon:Number(g.lon)};
    const list=this.nearbyAssetsForGps(g.lat,g.lon);
    const near=nearestFrom(this,origin,list,null);
    const nearest=near?.asset||null, nearestM=near?.m??Infinity;
    let next=null,nextM=Infinity;
    const circuit=patrolLineForAsset(nearest)||displayLine(this.currentCircuit||'');
    if(nearest){
      const nLine=compact(patrolLineForAsset(nearest));
      let active=this._gpsActivePool?.()||[];
      if(!active.length)active=(this.buildPatrolCircuitCandidates?.()||[]).filter(c=>compact(c.line)===nLine).map(c=>c.asset);
      const sameLine=filterPatrolAssets(active).filter(a=>!nLine||compact(patrolLineForAsset(a))===nLine);
      const sorted=sameLine.slice().sort(window.SearchEngine?.sortByStructure||(()=>0));
      let ni=sorted.findIndex(a=>sameAsset(a,nearest));
      const candidates=[];
      if(ni>0)candidates.push(sorted[ni-1]);
      if(ni>=0&&ni<sorted.length-1)candidates.push(sorted[ni+1]);
      const heading=Number(g.heading); let bestScore=Infinity;
      for(const c of candidates){
        const m=this.distanceM(origin,c); if(!Number.isFinite(m)||m<2)continue;
        const b=this.bearingDeg(origin,c); const diff=this.angleDiffDeg(heading,b);
        const score=(Number.isFinite(heading)?diff*7:0)+m;
        if(score<bestScore){bestScore=score;next=c;nextM=m;}
      }
      if(!next){
        for(const c of filterPatrolAssets(list)){
          if(sameAsset(c,nearest))continue;
          if(nLine&&compact(patrolLineForAsset(c))!==nLine)continue;
          const m=this.distanceM(origin,c); if(!Number.isFinite(m)||m<2)continue;
          const b=this.bearingDeg(origin,c); const diff=this.angleDiffDeg(heading,b);
          const score=(Number.isFinite(heading)?diff*8:0)+m;
          if(score<bestScore){bestScore=score;next=c;nextM=m;}
    ...
    const summary={ts:now,cacheKey,nearest,nearestM,next,nextM,circuit};
    this.gpsNearestCache=summary;
    return summary;
  };
  ...
  ME.nearestCircuitAnyDistance=function(){
    ...
    const now=Date.now();
    const list=this.buildPatrolCircuitCandidates?.()||[];
    const q=[Math.round(Number(g.lat)*10000),Math.round(Number(g.lon)*10000),this._patrolCircuitCandidateStamp||'','v240'].join('|');
    if(this._patrolNearestCircuitCache&&this._patrolNearestCircuitCache.q===q&&now-(this._patrolNearestCircuitCache.ts||0)<3500)return this._patrolNearestCircuitCache.hit;
    const origin={lat:Number(g.lat),lon:Number(g.lon)};
    let best=null,bestM=Infinity;
    for(const c of list){
      if(!c?.asset||!patrolLineForAsset(c.asset))continue;
      const m=this.distanceM(origin,c);
      if(Number.isFinite(m)&&m<bestM){best=c;bestM=m;}
    }
    const hit=best?{line:best.line,distanceM:bestM,asset:best.asset}:null;
  ...
  ME.updateGpsPanel=function(pos){
    ...
    try{
      const sum=this.gpsNearestSummary?.();
      if(!sum?.nearest){
        const nearest=document.getElementById('gpsNearestValue'); if(nearest)nearest.textContent='No transmission circuit asset nearby';
        const next=document.getElementById('gpsNextValue'); if(next)next.textContent='—';
        const ping=document.getElementById('gpsNearestPingBtn'); if(ping){ping.disabled=true;ping.title='No transmission circuit asset to ping';}
      }
    }catch(_){ }
    return out;
  };
