...
/* myMap v3.1.238: startup index restore/build without full text re-index every app reopen. */
(function(){
  const S=typeof window!=='undefined'?window.SearchEngine:SearchEngine;
  if(!S||S.__v237StartupIndexCache)return; S.__v237StartupIndexCache=true;
  S.restoreLightIndexCache=async function(cache={}){
    try{
      const assets=Array.isArray(window.App?.assets)?window.App.assets:[];
      if(!assets.length)return false;
      const idMap=new Map();
      for(let i=0;i<assets.length;i++){
        const a=assets[i]; if(!a||typeof a!=='object')continue;
        let id=String(a.id||'');
        if(!id&&this.assetStableId){try{id=String(this.assetStableId(a,i)||'');}catch(_e){}}
        if(!id)id='idx:'+i;
        if(!a.id)a.id=id;
        if(!idMap.has(id))idMap.set(id,a);
      }
      const resolve=(ids=[])=>{
        const out=[];
        for(const id of (Array.isArray(ids)?ids:[])){
          const a=idMap.get(String(id)); if(a)out.push(a);
        }
        return out;
      };
      this.lineMap=new Map();
      for(const [key,g] of (Array.isArray(cache.lineMap)?cache.lineMap:[])){
        const group={
          line:g?.line||'',rawLine:g?.rawLine||g?.line||'',assets:resolve(g?.assetIds),validGps:Number(g?.validGps||0),routeAssets:resolve(g?.routeIds),assetIds:new Set(g?.assetIds||[]),routeIds:new Set(g?.routeIds||[])
        };
        this.lineMap.set(key,group);
      }
      this.assetMap=new Map();
      const sourceIds=Array.isArray(cache.assetIds)&&cache.assetIds.length?cache.assetIds:Array.from(idMap.keys());
      for(const id of sourceIds){const a=idMap.get(String(id)); if(a)this.assetMap.set(String(id),a);}
      this.indexedAssetIds=new Set(this.assetMap.keys());
      this.searchDocs=Array.from(this.assetMap.keys());
      // Token index is deliberately not restored: it is the heaviest part of the old startup rebuild.
      // Searches still work through the existing full-data fallback; manual Speed index rebuild restores
      // full-token speed for the current session.
      this.tokenIndex=new Map();
      this.spatialIndex=new Map();
      for(const [key,ids] of (Array.isArray(cache.spatialIndex)?cache.spatialIndex:[]))this.spatialIndex.set(key,resolve(ids));
      this.kindIndex={reference:[],structure:[],transformer:[],misc:[],all:[]};
      for(const [key,ids] of Object.entries(cache.kindIndex||{}))this.kindIndex[key]=resolve(ids);
      if(!Array.isArray(this.kindIndex.all)||!this.kindIndex.all.length)this.kindIndex.all=Array.from(this.assetMap.values());
      this.docCache=new WeakMap();
      this.searchCache=new Map();
      this.recoveryLineCache=new Map();
      this.poleDetailMap=new Map();
      this.poleDetailMapBuilt=false;
      this.circuitPathIndex=new Map(Array.isArray(cache.circuitPathIndex)?cache.circuitPathIndex:[]);
      this.circuitEndpointPathIndex=new Map();
      for(const [key,lines] of (Array.isArray(cache.circuitEndpointPathIndex)?cache.circuitEndpointPathIndex:[]))this.circuitEndpointPathIndex.set(key,new Set(lines||[]));
      this.referencePointsByCode=new Map();
      for(const [key,ids] of (Array.isArray(cache.referencePointsByCode)?cache.referencePointsByCode:[]))this.referencePointsByCode.set(key,resolve(ids));
      this.referencePoints=resolve(cache.referencePoints||[]);
      this.circuitPathStats=cache.circuitPathStats||null;
      this.spatialGridSize=0.025;
      this.indexStats={
        ...(cache.indexStats||{}),
        restoredAt:new Date().toISOString(),
        restoredFromStartupCache:true,
        lazyTextSearch:true,
        assetsIndexed:this.assetMap.size,
        lineGroups:this.lineMap.size,
        circuitPathGroups:this.circuitPathIndex.size,
        tokenCount:0,
        spatialCells:this.spatialIndex.size,
        schema:window.App?.schema||{},
        mode:'saved startup index · lazy text search'
      };
      return true;
    }catch(err){
      try{window.Diagnostics?.log?.('Light index restore failed',String(err?.message||err));}catch(_e){}
      return false;
    }
  };
  S.buildLightIndexAsync=async function(reason='Startup saved index build'){
    if(this.indexRunning)throw new Error('Index rebuild already running');
    this.indexRunning=true;
    this.indexCancelRequested=false;
    this.indexCancelReason='';
    try{
      this.lineMap=new Map(); this.assetMap=new Map(); this.searchDocs=[]; this.tokenIndex=new Map(); this.spatialIndex=new Map(); this.indexedAssetIds=new Set();
      this.spatialGridSize=0.025; this.resetFastIndexes();
      const start=Date.now();
      const assets=Array.isArray(window.App?.assets)?window.App.assets:[];
      const total=assets.length||1;
      UI?.progress?.(true,reason,`Building saved startup index for ${total.toLocaleString()} loaded records`,70);
      await new Promise(r=>setTimeout(r,0));
      this.assertIndexNotCancelled?.();
      try{
        this.resolveDualCircuits?.(assets);
        this.linkConductorSections?.(assets);
        this.buildReferenceIndex?.(assets);
      }catch(err){try{Diagnostics?.log?.('Light startup reference resolver skipped',String(err?.message||err));}catch(_e){}}
      let indexed=0,gpsIndexed=0;
      for(let i=0;i<assets.length;i++){
        const a=assets[i];
        try{
          if(!a||typeof a!=='object')continue;
          try{this.repairStructureIdentity?.(a);}catch(_e){}
          if(this.isHVCrossingAsset?.(a)||this.isUtilityAsset?.(a))continue;
          const stable=this.assetStableId?this.assetStableId(a,indexed):(a.id||('idx:'+i));
          if(!a.id)a.id=stable;
          this.assetMap.set(stable,a);
          this.indexedAssetIds.add(stable);
          this.searchDocs.push(stable);
          const rawLine=a.line||a.substation||'';
          const lines=this.lineMapLinesForAsset?.(a,rawLine)||[];
          for(const line of lines){
            if(!line)continue;
            ...
            if(!group.assetIds.has(stable)){
              group.assetIds.add(stable);
              group.assets.push(a);
              if(Number.isFinite(Number(a.lat))&&Number.isFinite(Number(a.lon))&&a.kind!=='circuit')group.validGps++;
            }
            if(a.routeCoords?.length&&!group.routeIds.has(stable)){group.routeIds.add(stable);group.routeAssets.push(a);}
          }
          this.addKindIndex?.(a);
          if(!this.isConductorSpanAsset?.(a)&&Number.isFinite(Number(a.lat))&&Number.isFinite(Number(a.lon))&&a.kind!=='circuit'){this.addSpatialAsset?.(a);gpsIndexed++;}
          indexed++;
        }catch(err){try{Diagnostics?.log?.('Skipped asset during light startup index',String(err?.message||err));}catch(_e){}}
        if(i%1200===0){
          this.assertIndexNotCancelled?.();
          UI?.progress?.(true,reason,`Prepared ${i.toLocaleString()} / ${total.toLocaleString()} records · ${this.lineMap.size.toLocaleString()} circuits`,70+Math.min(20,Math.round((i/total)*20)));
          await new Promise(r=>setTimeout(r,0));
          this.assertIndexNotCancelled?.();
        }
      }
      let sorted=0, groups=Array.from(this.lineMap.values());
      for(const group of groups){group.assets.sort(this.sortByStructure); if(++sorted%100===0)await new Promise(r=>setTimeout(r,0));}
      try{await this.buildCircuitPathIndexAsync?.(assets,reason);}catch(err){if(err?.name==='AbortError')throw err;try{Diagnostics?.log?.('Light startup path optimiser skipped',String(err?.message||err));}catch(_e){}}
      try{this.buildReferenceIndex?.(assets); this.enrichReferenceIndexFromLineEndpoints?.(assets);}catch(err){try{Diagnostics?.log?.('Light startup endpoint enrichment skipped',String(err?.message||err));}catch(_e){}}
      this.indexStats={
        rebuiltAt:new Date().toISOString(),ms:Date.now()-start,assetsIndexed:indexed,gpsIndexed,lineGroups:this.lineMap.size,circuitPathGroups:this.circuitPathIndex?.size||0,
        tokenCount:0,tokenLinks:0,spatialCells:this.spatialIndex.size,schema:window.App?.schema||{},async:true,lazyTextSearch:true,mode:'light startup index',pass2Index:this.pass2IndexVersion,
        kindIndexCounts:Object.fromEntries(Object.entries(this.kindIndex||{}).map(([k,v])=>[k,Array.isArray(v)?v.length:0]))
      };
      window.App.indexHealth=window.App.indexHealth||{mode:'file-level',queue:[],files:[]};
      window.App.indexHealth.startupIndex={status:'rebuilt',rebuiltAt:this.indexStats.rebuiltAt,assetCount:indexed,fileCount:(window.App.files||[]).length,mode:'light startup index'};
      UI?.progress?.(true,reason,`Startup index ready · ${indexed.toLocaleString()} records · ${this.lineMap.size.toLocaleString()} circuits`,96);
      await new Promise(r=>setTimeout(r,0));
  ...
  const oldRebuild=S.rebuild?.bind(S);
  if(oldRebuild){
    S.rebuild=function(){
      const out=oldRebuild.apply(this,arguments);
      try{window.StorageEngine?.saveSearchIndexCache?.('manual/full rebuild');}catch(_e){}
      return out;
    };
  }
  const oldRebuildAsync=S.rebuildAsync?.bind(S);
  if(oldRebuildAsync){
    S.rebuildAsync=async function(){
      const out=await oldRebuildAsync.apply(this,arguments);
      try{await window.StorageEngine?.saveSearchIndexCache?.('async/full rebuild');}catch(_e){}
      return out;
    };
  }
  if(window.ImportEngine?.importFiles&&!window.ImportEngine.__v237SaveIndexAfterImport){
    window.ImportEngine.__v237SaveIndexAfterImport=true;
    const oldImport=window.ImportEngine.importFiles.bind(window.ImportEngine);
    window.ImportEngine.importFiles=async function(){
      const out=await oldImport.apply(this,arguments);
      try{await window.StorageEngine?.saveSearchIndexCache?.('import complete');}catch(_e){}
      return out;
    };
  }
})();
...
/* myMap v3.1.238: fast scoped fallback + transmission-only quick search.
   Prevents Search Transmission from walking every imported distribution/utility record
   when the saved startup index is running in lazy text-search mode. */
(function(){
  const S=typeof window!=='undefined'?window.SearchEngine:null;
  if(!S||S.__v238FastTransmissionSearch)return; S.__v238FastTransmissionSearch=true;
  const stable=(ctx,a,i=0)=>{try{return String(ctx.assetStableId?ctx.assetStableId(a,i):(a?.id||a?.label||a?.structure||a?.equip||('asset_'+i))||'');}catch(_e){return String(a?.id||a?.label||a?.structure||a?.equip||('asset_'+i));}};
  S.fastScopeFallback=function(scopeHint){
    const kind=this.kindIndex||{};
    const s={transmission:true,dxPoles:false,transformers:false,misc:false,...(scopeHint||{})};
    const out=[]; const seen=new Set();
    const add=(list)=>{
      if(!Array.isArray(list))return;
      for(let i=0;i<list.length;i++){
        const a=list[i]; if(!a||typeof a!=='object')continue;
        const id=stable(this,a,i); if(id&&seen.has(id))continue;
        if(id)seen.add(id); out.push(a);
      }
    };
    if(s.transmission!==false)add(kind.structure);
    if(s.transformers)add(kind.transformer);
    if(s.dxPoles||s.misc)add(kind.misc);
    add(kind.reference);
    if(out.length)return out;
    return this.filterCandidatesByScope?this.filterCandidatesByScope((window.App?.assets||[]),scopeHint):(window.App?.assets||[]);
  };
  S.candidateAssets=function(cq,ws,opts={}){
    const scopedFallback=()=>this.fastScopeFallback?this.fastScopeFallback(opts.scopeHint):(this.filterCandidatesByScope?this.filterCandidatesByScope((window.App?.assets||[]),opts.scopeHint):(window.App?.assets||[]));
    if(!this.searchDocs||!this.searchDocs.length||!this.tokenIndex||!this.tokenIndex.size)return scopedFallback();
    if(!cq)return scopedFallback();
    if(String(cq).length<2){
      const refs=this.kindIndex?.reference;
      return Array.isArray(refs)&&refs.length?refs:(window.App?.assets||[]).filter(a=>this.isReferencePointAsset?.(a));
    }
    const found=new Set();
    let probes=[cq,...(ws||[]).map(w=>this.compact(w)).filter(Boolean)]
      .filter(p=>p&&p.length>=2&&!/^\d{1,2}$/.test(p)&&!/^0+$/.test(p));
    ...
    if(found.size<60){
      for(const [tok,set] of this.tokenIndex){
        if(tok.length<2)continue;
        if(activeProbes.some(p=>p&&tok.includes(p)))addSet(set);
        if(found.size>1200)break;
      }
    }
    return found.size?Array.from(found):scopedFallback();
  };
  S.quickReferenceResults=function(cq,ws,limit){
    const refs=Array.isArray(this.kindIndex?.reference)?this.kindIndex.reference:[];
    const rows=[];
    for(const a of refs){
      let score=0;
      try{score=this.referenceQueryMatchLevel?.(a,cq,ws)||0;}catch(_e){score=0;}
      if(!score){
        const raw=a?.raw||{};
        const text=this.compact([a?.label,a?.substation,a?.address,a?.category,raw.ABBREVIATION,raw.SEARCH_FIELD,raw.SUBSTATION,raw.DEPOT_NAME].join(' '));
        if(text&&cq&&(text.startsWith(cq)||text.includes(cq)))score=text.startsWith(cq)?3:1;
      }
      if(score>0){
        rows.push({type:'asset',score:700+score*40,asset:a,title:a.label||a.substation||a.equip||'Reference point',subtitle:this.subtitle?.(a)||a.kind||'',kind:a.kind||'asset'});
        if(rows.length>=limit)break;
      }
    }
    return rows;
  };
  S.lightStructureScore=function(a,cq,ws,qPoles,alphaTerms){
    if(!a||typeof a!=='object')return 0;
    let s=0;
    const refs=this.lineRefsForAsset?.(a,true)||[];
    const poles=[]; const addPole=(v)=>{const p=this.poleIdParts?.(v); if(p&&!poles.some(x=>x.norm===p.norm))poles.push(p);};
    addPole(a?.poleNumber); addPole(a?.label); addPole(a?.structure); addPole(a?.gisLabel);
    for(const r of refs)addPole(r.pole);
    const lineText=this.compact([a?.line,a?.label,a?.structure,a?.gisLabel,...refs.flatMap(r=>[r.line,r.pole,`${r.line||''}${r.pole||''}`])].join(' '));
    if(Array.isArray(qPoles)&&qPoles.length){
      for(const qp of qPoles){
        const exact=poles.some(ap=>this.poleIdMatches?.(ap.norm,qp));
        if(exact)s+=1500+(qp.suffix&&poles.some(ap=>ap.num===qp.num&&ap.suffix===qp.suffix)?500:0);
      }
      if(s>0&&alphaTerms?.length){
        if(alphaTerms.some(t=>t&&lineText.includes(t)))s+=550;
        else s-=450;
      }
    }else if(cq&&String(cq).length>=3){
      if(lineText===cq)s+=400;
      else if(lineText.startsWith(cq))s+=240;
      else if(lineText.includes(cq))s+=120;
      if(alphaTerms?.length&&alphaTerms.some(t=>t&&lineText.includes(t)))s+=60;
    }
    if(s>0&&Number.isFinite(Number(a.lat))&&Number.isFinite(Number(a.lon)))s+=4;
    return s;
  };
  S.searchTransmissionFast=function(q,limit=35){
    const query=String(q||'').trim(); if(!query)return [];
    const cq=this.compact(query), ws=this.words(query), seen=new Set(), rows=[];
    const add=(r)=>{if(!r)return; const k=this.resultDedupKey?.(r)||`${r.type||'r'}:${r.title||r.line||''}`; if(k&&seen.has(k))return; if(k)seen.add(k); rows.push(r);};
    let exactLineQuery=false;
    try{for(const g of (this.lineMap?.values?.()||[])){if(this.compact(g?.line||'')===cq){exactLineQuery=true;break;}}}catch(_e){}
    const stemPole=exactLineQuery?null:this.compactLineStemPoleQuery?.(query);
    if(stemPole){
      const exact=(this.compactLinePoleResultsFast?.(stemPole)||this.compactLinePoleResults?.(stemPole)||[]);
      for(const r of exact)add(r);
    ...
    try{
      for(const group of (this.lineMap?.values?.()||[])){
        if(!group||!this.isDisplayableTransmissionCircuitLine?.(group.line))continue;
        const score=this.scoreLine?.(cq,ws,group)||0;
        if(score>=80){
          const highest=(this.quickHighestForGroup?.(group) ?? this.highestPoleForLineValue?.(group.line,group.assets));
          const highestLabel=highest!==null&&highest!==undefined?Number(highest).toLocaleString():'unknown';
          circuitRows.push({type:'circuit',score:score+20,line:group.line,group,title:group.line,subtitle:`Highest structure: ${highestLabel} · ${Number(group.validGps||0).toLocaleString()} mapped`,kind:'circuit'});
        }
      }
    }catch(_e){}
    circuitRows.sort((a,b)=>b.score-a.score||String(a.title||'').localeCompare(String(b.title||'')));
    for(const r of circuitRows)add(r);
    for(const r of this.quickReferenceResults?.(cq,ws,limit)||[])add(r);
    const qPoles=exactLineQuery?[]:(this.queryPoleIdCandidates?.(cq,ws)||[]);
    const poleSpecific=!exactLineQuery&&(this.queryHasPoleSpecificNumber?.(query,ws)||qPoles.length>0);
    const alphaTerms=(ws||[]).map(w=>this.compact(w)).filter(w=>w&&/[A-Z]/.test(w)&&!/^(POLE|TOWER|STRUCTURE|LINE|CIRCUIT|TRANSMISSION|TX)$/i.test(w));
    if(poleSpecific||(!circuitRows.length&&cq.length>=4)){
      let candidates=[];
      if(stemPole?.groups?.length)candidates=stemPole.groups.flatMap(g=>g.assets||[]);
      else if(qPoles.length&&this.ensureGlobalStructurePoleLookup){
        const lookup=this.ensureGlobalStructurePoleLookup(); const cSeen=new Set();
        for(const qp of qPoles){
          const key=this.simplePoleKey?.(qp)||this.simplePoleKey?.(qp?.norm)||'';
          for(const a of (lookup.get(key)||[])){const id=stable(this,a,0); if(id&&cSeen.has(id))continue; if(id)cSeen.add(id); candidates.push(a);}
        }
      }
      else if(circuitRows.length&&alphaTerms.length)candidates=circuitRows.slice(0,4).flatMap(r=>r.group?.assets||[]);
      else candidates=Array.isArray(this.kindIndex?.structure)?this.kindIndex.structure:[];
      const assetRows=[]; const localSeen=new Set();
      for(let i=0;i<candidates.length;i++){
        const a=candidates[i]; if(!a||typeof a!=='object')continue;
        const id=stable(this,a,i); if(id&&localSeen.has(id))continue; if(id)localSeen.add(id);
        try{if(this.passesFilters&&!this.passesFilters(a))continue;}catch(_e){}
        const score=this.lightStructureScore?.(a,cq,ws,qPoles,alphaTerms)||0;
        if(score>0)assetRows.push({type:'asset',score,asset:a,title:a.label||a.structure||a.equip||a.line||'Asset',subtitle:this.subtitle?.(a)||a.kind||'asset',kind:a.kind||'asset'});
        if(assetRows.length>250)break;
      }
      assetRows.sort((a,b)=>b.score-a.score||String(a.title||'').localeCompare(String(b.title||'')));
      for(const r of assetRows)add(r);
    }
    rows.sort((a,b)=>{
      if(a.type!==b.type){
        if(a.type==='asset'&&poleSpecific)return -1;
        if(b.type==='asset'&&poleSpecific)return 1;
        if(a.type==='circuit')return -1;
        if(b.type==='circuit')return 1;
      }
      return (b.score||0)-(a.score||0)||String(a.title||'').localeCompare(String(b.title||''));
    });
    return rows.slice(0,limit);
  };
})();

/* myMap v3.1.238: optimise exact structure lookup and highest-structure labels. */
(function(){
  const S=typeof window!=='undefined'?window.SearchEngine:null;
  if(!S||S.__v238TxSearchMicroOptimise)return; S.__v238TxSearchMicroOptimise=true;
  S.quickPolePartsForAsset=function(a){
    const raw=a?.raw||{};
    const vals=[a?.poleNumber,a?.label,a?.structure,a?.gisLabel,raw.NAMEPLATE_ID,raw.POLE_NUMBER,raw.POLE_NO,raw.STRUCTURE_LABEL,raw.TRMSN_LINE_GIS_LABEL].filter(Boolean);
    const out=[];
    for(const v of vals){const p=this.poleIdParts?.(v); if(p&&!out.some(x=>x.norm===p.norm))out.push(p);}
    return out;
  };
  S.quickLineTextForAsset=function(a,includeRefs=false){
    const raw=a?.raw||{};
    const base=[a?.line,a?.label,a?.structure,a?.gisLabel,raw.LINE_NAME,raw.LINE_NAME_1,raw.LINE_NAME_2,raw.CIRCUIT,raw.CIRCUIT_NAME,raw.TRMSN_LINE_GIS_LABEL].filter(Boolean);
    if(includeRefs){try{base.push(...(this.lineRefsForAsset?.(a,true)||[]).flatMap(r=>[r.line,r.pole,`${r.line||''}${r.pole||''}`]));}catch(_e){}}
    return this.compact(base.join(' '));
  };
  S.quickHighestForGroup=function(group){
    if(!group)return null;
    if(Number.isFinite(group.__mymapHighestPole))return group.__mymapHighestPole;
    let max=null;
    for(const a of (group.assets||[])){
      const parts=this.quickPolePartsForAsset?.(a)||[];
      for(const p of parts){if(max===null||p.num>max)max=p.num;}
    }
    group.__mymapHighestPole=max;
    return max;
  };
  S.compactLinePoleResultsFast=function(stemPole){
    if(!stemPole||!Array.isArray(stemPole.groups)||!stemPole.groups.length)return [];
    const rows=[]; const seen=new Set();
    for(const group of stemPole.groups){
      const pole=(stemPole.groupPoleMap&&stemPole.groupPoleMap.get(this.compact(group.line)))||stemPole.pole;
      if(!pole)continue;
      for(const a of (group.assets||[])){
        if(!a||typeof a!=='object')continue;
        let match=false;
        const parts=this.quickPolePartsForAsset?.(a)||[];
        match=parts.some(ap=>this.poleIdMatches?.(ap.norm,pole));
        if(!match){try{match=this.assetMatchesLinePole?.(a,group.line,pole);}catch(_e){match=false;}}
        if(!match)continue;
        try{if(this.passesFilters&&!this.passesFilters(a))continue;}catch(_e){}
        const r={type:'asset',score:5000,asset:a,title:a.label||a.structure||a.equip||a.line||'Asset',subtitle:this.subtitle?.(a)||a.kind||'asset',kind:a.kind||'asset'};
        const k=this.resultDedupKey?.(r)||String(r.title||''); if(k&&seen.has(k))continue; if(k)seen.add(k);
        rows.push(r);
      }
    }
    rows.sort((a,b)=>{
      const ap=this.poleIdSortValue?.(a.asset?.poleNumber||a.asset?.label||a.asset?.structure)||{num:0,suffix:''};
      const bp=this.poleIdSortValue?.(b.asset?.poleNumber||b.asset?.label||b.asset?.structure)||{num:0,suffix:''};
      if(ap.num!==bp.num)return ap.num-bp.num;
      if(ap.suffix!==bp.suffix)return String(ap.suffix||'').localeCompare(String(bp.suffix||''));
      return String(a.title||'').localeCompare(String(b.title||''));
    });
  ...
  const oldLight=S.lightStructureScore;
  S.lightStructureScore=function(a,cq,ws,qPoles,alphaTerms){
    if(!a||typeof a!=='object')return 0;
    const directPoles=this.quickPolePartsForAsset?.(a)||[];
    const alpha=Array.isArray(alphaTerms)?alphaTerms.filter(Boolean):[];
    let s=0;
    if(Array.isArray(qPoles)&&qPoles.length){
      for(const qp of qPoles){
        const exact=directPoles.some(ap=>this.poleIdMatches?.(ap.norm,qp));
        if(exact)s+=1500+(qp.suffix&&directPoles.some(ap=>ap.num===qp.num&&ap.suffix===qp.suffix)?500:0);
      }
      if(s>0){
        if(alpha.length){
          let lineText=this.quickLineTextForAsset?.(a,false)||'';
          if(!alpha.some(t=>lineText.includes(t)))lineText=this.quickLineTextForAsset?.(a,true)||lineText;
          if(alpha.some(t=>lineText.includes(t)))s+=550;
          else s-=450;
        }
        if(Number.isFinite(Number(a.lat))&&Number.isFinite(Number(a.lon)))s+=4;
        return s;
      }
      return 0;
    }
    if(cq&&String(cq).length>=3){
      let lineText=this.quickLineTextForAsset?.(a,false)||'';
      if(lineText===cq)s+=400;
      else if(lineText.startsWith(cq))s+=240;
      else if(lineText.includes(cq))s+=120;
      if(!s&&alpha.length){lineText=this.quickLineTextForAsset?.(a,true)||lineText; if(alpha.some(t=>lineText.includes(t)))s+=80;}
      if(s>0&&Number.isFinite(Number(a.lat))&&Number.isFinite(Number(a.lon)))s+=4;
      return s;
    }
    return oldLight?oldLight.call(this,a,cq,ws,qPoles,alphaTerms):0;
  };
})();

/* myMap v3.1.238: direct pole lookup map for fast line+structure searches. */
(function(){
  const S=typeof window!=='undefined'?window.SearchEngine:null;
  if(!S||S.__v238DirectPoleLookup)return; S.__v238DirectPoleLookup=true;
  S.simplePoleKey=function(v){
    if(v&&typeof v==='object'&&Number.isFinite(Number(v.num)))return String(Number(v.num))+(v.suffix||'')+(v.branchPrefix?'/'+v.branchPrefix:'')+(v.branchNum?'/'+v.branchNum:'');
    let t=String(v??'').toUpperCase().replace(/\s+/g,'').replace(/[^A-Z0-9/]/g,'');
    if(!t)return '';
    let m=t.match(/^0*(\d{1,6})([A-Z]{0,3})(?:\/([A-Z]{1,4}|0*\d{1,6}[A-Z]{0,3}))?$/i);
    if(m)return String(Number(m[1]))+(m[2]||'')+(m[3]?'/'+String(m[3]).replace(/^0+(?=\d)/,''):'');
    m=t.match(/(?:^|[^A-Z0-9])0*(\d{1,6})([A-Z]{0,3})$/i);
    return m?String(Number(m[1]))+(m[2]||''):'';
  };
  S.ensureGroupPoleLookup=function(group){
    if(!group)return new Map();
    if(group.__mymapPoleLookup instanceof Map)return group.__mymapPoleLookup;
    const map=new Map();
    const add=(key,a)=>{if(!key||!a)return; if(!map.has(key))map.set(key,[]); map.get(key).push(a);};
    for(const a of (group.assets||[])){
      const raw=a?.raw||{};
      const vals=[a?.poleNumber,a?.structure,a?.rawStructure,raw.NAMEPLATE_ID,raw.nameplate_id,raw.POLE_NUMBER,raw.POLE_NO,raw.STRUCTURE_ID].filter(Boolean);
      for(const v of vals)add(this.simplePoleKey?.(v),a);
    }
    group.__mymapPoleLookup=map;
    return map;
  };
  S.compactLinePoleResultsFast=function(stemPole){
    if(!stemPole||!Array.isArray(stemPole.groups)||!stemPole.groups.length)return [];
    ...
    const addRow=(a)=>{
      if(!a||typeof a!=='object')return;
      try{if(this.passesFilters&&!this.passesFilters(a))return;}catch(_e){}
      const r={type:'asset',score:5000,asset:a,title:a.label||a.structure||a.equip||a.line||'Asset',subtitle:this.subtitle?.(a)||a.kind||'asset',kind:a.kind||'asset'};
      const k=this.resultDedupKey?.(r)||String(r.title||''); if(k&&seen.has(k))return; if(k)seen.add(k); rows.push(r);
    };
    for(const group of stemPole.groups){
      const pole=(stemPole.groupPoleMap&&stemPole.groupPoleMap.get(this.compact(group.line)))||stemPole.pole;
      const key=this.simplePoleKey?.(pole)||this.simplePoleKey?.(pole?.norm)||'';
      const direct=key?(this.ensureGroupPoleLookup?.(group).get(key)||[]):[];
      if(direct.length){for(const a of direct)addRow(a); continue;}
      for(const a of (group.assets||[])){
        let match=false;
        try{match=this.assetMatchesLinePole?.(a,group.line,pole);}catch(_e){match=false;}
        if(match)addRow(a);
      }
    }
    rows.sort((a,b)=>String(a.title||'').localeCompare(String(b.title||''),undefined,{numeric:true,sensitivity:'base'}));
    return rows;
  };
})();

/* myMap v3.1.238: global direct pole lookup for structure-number-only searches. */
(function(){
  const S=typeof window!=='undefined'?window.SearchEngine:null;
  if(!S||S.__v238GlobalPoleLookup)return; S.__v238GlobalPoleLookup=true;
  S.ensureGlobalStructurePoleLookup=function(){
    if(this.__mymapGlobalStructurePoleLookup instanceof Map)return this.__mymapGlobalStructurePoleLookup;
    const map=new Map();
    const add=(key,a)=>{if(!key||!a)return; if(!map.has(key))map.set(key,[]); map.get(key).push(a);};
    const list=Array.isArray(this.kindIndex?.structure)?this.kindIndex.structure:[];
    for(const a of list){
      const raw=a?.raw||{};
      for(const v of [a?.poleNumber,a?.structure,a?.rawStructure,raw.NAMEPLATE_ID,raw.nameplate_id,raw.POLE_NUMBER,raw.POLE_NO,raw.STRUCTURE_ID].filter(Boolean))add(this.simplePoleKey?.(v),a);
    }
    this.__mymapGlobalStructurePoleLookup=map;
    return map;
  };
})();

/* myMap v3.1.238: clear fast lookup caches when indexes rebuild. */
(function(){
  const S=typeof window!=='undefined'?window.SearchEngine:null;
  if(!S||S.__v238ClearFastLookupCaches)return; S.__v238ClearFastLookupCaches=true;
  const oldReset=S.resetFastIndexes;
  S.resetFastIndexes=function(){
    const out=oldReset?oldReset.apply(this,arguments):undefined;
    try{this.__mymapGlobalStructurePoleLookup=null;}catch(_e){}
    try{for(const g of (this.lineMap?.values?.()||[])){delete g.__mymapPoleLookup; delete g.__mymapHighestPole;}}catch(_e){}
    return out;
  };
})();
