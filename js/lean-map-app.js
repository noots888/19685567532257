var LeanMapApp={
  ...
  async boot(){
    try{
      ...
      const savedCount=(App.assets||[]).length;
      UI.progress(true,'Loading saved startup index…',`${savedCount.toLocaleString()} records loaded`,62);
      let indexRestored=false;
      try{indexRestored=!!(await StorageEngine.loadSearchIndexCache?.());}catch(_e){indexRestored=false;}
      if(!indexRestored){
        UI.progress(true,'Preparing startup index…',`${savedCount.toLocaleString()} saved records · first run after import/update only`,66);
        if(SearchEngine.buildLightIndexAsync)await SearchEngine.buildLightIndexAsync('Startup saved index build');
        else await (SearchEngine.rebuildAsync?SearchEngine.rebuildAsync('Startup data index rebuild'):SearchEngine.rebuild());
        try{await StorageEngine.saveSearchIndexCache?.('startup build');}catch(_e){}
      }
  ...
  renderDataPanel(preserveScroll=false){
    ...
    if(cat==='app'){
      ...
      const cacheStatus=StorageEngine?.indexCacheStatus||App.indexHealth?.startupIndex?.status||'not checked';
      const cacheSaved=App.indexHealth?.startupIndex?.savedAt||App.indexHealth?.startupIndex?.rebuiltAt||App.indexHealth?.startupIndex?.loadedAt||'';
      const appRows=`<div class="mini-row"><b>Current version</b><span>${UI.esc(App.versionShort||App.version)}</span></div><div class="mini-row"><b>Build</b><span>${UI.esc(App.version)}</span></div><div class="mini-row"><b>Installed</b><span>${installed?'yes':'not installed'}</span></div><div class="mini-row"><b>Startup index</b><span>${UI.esc(cacheStatus)}</span></div><div class="mini-row"><b>Index saved</b><span>${UI.esc(this.fmtDate(cacheSaved)||'not yet')}</span></div><div class="mini-row"><b>Text search</b><span>${idx.lazyTextSearch?'lazy after restart':'full speed this session'}</span></div>`;
      const indexNote=idx.lazyTextSearch?'The app skips the heavy full text re-index on reopen. Search still works; use Rebuild speed index on Summary if you want the fastest text search for this session.':'Full speed index is active for this session.';
      b.innerHTML=`<button type="button" class="data-download-action-btn data-download-only-btn" data-install-app="1">${installed?'App installed':'Download app'}</button>${this.sectionHtml('appCurrentVersion','App version',appRows,'current app / startup index',true)}<div class="data-card"><b>Index behaviour</b><p>${UI.esc(indexNote)}</p><small>App version updates no longer force a full 500k-record re-index unless the saved data/index signature changes.</small></div>`;
    ...
    const idx=SearchEngine.indexStats||{};
    const searchMode=idx.lazyTextSearch?'saved startup index / lazy text search':'indexed';
    const speedStatus=`<div class="mini-row"><b>Startup map drawing</b><span>empty</span></div><div class="mini-row"><b>Search mode</b><span>${UI.esc(searchMode)}</span></div><div class="mini-row"><b>Indexed assets</b><span>${Number(idx.assetsIndexed||assets.length||0).toLocaleString()}</span></div><div class="mini-row"><b>Search tokens</b><span>${Number(idx.tokenCount||0).toLocaleString()}</span></div><div class="mini-row"><b>Startup index</b><span>${UI.esc(StorageEngine?.indexCacheStatus||App.indexHealth?.startupIndex?.status||'not checked')}</span></div><div class="mini-row"><b>Last rebuild</b><span>${Number(idx.ms||0).toLocaleString()} ms</span></div>`;
    const kindRows=this.countsBy(assets,x=>x.kind||x.category||'Other').slice(0,18).map(([k,n])=>`<div class="mini-row"><b>${UI.esc(k)}</b><span>${n.toLocaleString()}</span></div>`).join('');
...
/* myMap v3.1.238: non-freezing Search Transmission panel.
   Runs transmission search through the fast transmission-only path and renders after
   the UI has had a frame to show the search state. */
(function(){
  const APP=typeof window!=='undefined'?window.LeanMapApp:null;
  if(!APP||APP.__v238FastTransmissionSearchPanel)return; APP.__v238FastTransmissionSearchPanel=true;
  const oldRun=APP.runAssetSearch;
  const esc=(v)=>{try{return window.UI?.esc?UI.esc(v):String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}catch(_e){return String(v??'');}};
  APP.runAssetSearch=function(){
    const distribution=this.assetSearchMode==='distribution';
    if(distribution)return oldRun?oldRun.apply(this,arguments):undefined;
    const q=document.getElementById('assetSearchInput')?.value||'';
    const box=document.getElementById('assetSearchResults');
    const btn=document.getElementById('runAssetSearchBtn');
    if(!box)return;
    if(!q.trim()){
      box.innerHTML='<div class="tiny-note">Search transmission circuits/structures plus depots, substations and terminals. Distribution, transformers, pillars, streetlights and utilities are excluded.</div>';
      return;
    }
    const token=Date.now()+':'+Math.random();
    this.__txSearchToken=token;
    box.innerHTML='<div class="tiny-note">Searching transmission only…</div>';
    if(btn){btn.disabled=true;btn.textContent='Searching…';}
    setTimeout(()=>{
      if(this.__txSearchToken!==token)return;
      let rows=[];
      try{
        rows=(window.SearchEngine?.searchTransmissionFast?SearchEngine.searchTransmissionFast(q,40):SearchEngine.search(q,40,{scopeHint:{transmission:true,dxPoles:false,transformers:false,misc:false},resultFilter:(r)=>r?.type==='asset'&&this.isTransmissionSearchAsset?.(r.asset)}))||[];
      }catch(err){try{window.Diagnostics?.capture?.(err);}catch(_e){} rows=[];}
      if(this.__txSearchToken!==token)return;
      if(btn){btn.disabled=false;btn.textContent='Search';}
      if(!rows.length){box.innerHTML='<div class="tiny-note">No transmission results.</div>';return;}
      const shown=rows.slice(0,35);
      box.innerHTML=shown.map((r,i)=>`<div class="result-card"><b>${esc(r.title||r.line||'Result')}</b><span>${esc(r.subtitle||r.kind||'Transmission result')}</span><button type="button" data-i="${i}">Map</button></div>`).join('')+(rows.length>shown.length?`<div class="tiny-note">Showing ${shown.length} of ${rows.length} fast results. Refine the search to narrow it down.</div>`:'');
      box.querySelectorAll('button[data-i]').forEach(b=>b.addEventListener('click',async()=>{
        const r=shown[Number(b.dataset.i)]; this.closeAssetSearch?.();
        try{
          if(r?.type==='circuit'||r?.line)await window.MapEngine?.showCircuit?.(r.line);
          else if(r?.asset)window.MapEngine?.showAsset?.(r.asset);
          window.UI?.refreshCounts?.();
        }catch(err){try{window.Diagnostics?.capture?.(err);}catch(_e){} window.UI?.toast?.('Map result failed.');}
      }));
    },30);
  };
})();
