...
if(typeof window!=='undefined')window.StorageEngine=StorageEngine;

/* myMap v3.1.238: saved startup index cache.
   Keeps app version changes separate from data-index refreshes so reopening the PWA
   does not rebuild every saved record unless the stored data/index signature changed. */
(function(){
  const SE=(typeof window!=='undefined'&&window.StorageEngine)?window.StorageEngine:StorageEngine;
  if(!SE||SE.__v237StartupIndexCache)return; SE.__v237StartupIndexCache=true;
  SE.searchIndexCacheKey='searchIndexCache:v237:light';
  SE.searchIndexCacheVersion='light-startup-index-v1';
  SE.indexCacheStatus='not checked';
  SE.indexCacheMessage='';
  SE._assetIdForCache=function(asset,idx=0){
    if(!asset||typeof asset!=='object')return '';
    let id=String(asset.id||'');
    if(!id&&typeof window!=='undefined'&&window.SearchEngine?.assetStableId){
      try{id=String(window.SearchEngine.assetStableId(asset,idx)||'');}catch(_e){}
    }
    if(!id)id='idx:'+idx;
    try{if(!asset.id)asset.id=id;}catch(_e){}
    return id;
  };
  SE.searchIndexFileHash=function(){
    const files=Array.isArray(window.App?.files)?window.App.files:[];
    const rows=files.map(f=>[
      f?.name||'',f?.size||0,f?.count||0,f?.dxPoleCount||0,f?.featuresRead||0,f?.assetsIndexed||0,
      f?.storageKey||'',f?.chunkCount||0,f?.dxPoleStorageKey||'',f?.dxPoleChunkCount||0,
      f?.indexStatus||f?.status||'',f?.parserVersion||''
    ].join('|')).sort().join('\n');
    return this.hash(rows);
  };
  SE.searchIndexSignature=function(){
    const app=window.App||{};
    const S=window.SearchEngine||{};
    return {
      cacheVersion:this.searchIndexCacheVersion,
      assetCount:Array.isArray(app.assets)?app.assets.length:0,
      fileCount:Array.isArray(app.files)?app.files.length:0,
      fileHash:this.searchIndexFileHash(),
      searchIndex:app.schema?.searchIndex||'',
      spatialIndex:app.schema?.spatialIndex||'',
      pass2Index:S.pass2IndexVersion||''
    };
  };
  SE.sameSearchIndexSignature=function(a={},b={}){
    return !!a&&!!b&&a.cacheVersion===b.cacheVersion&&a.assetCount===b.assetCount&&a.fileCount===b.fileCount&&a.fileHash===b.fileHash&&a.searchIndex===b.searchIndex&&a.spatialIndex===b.spatialIndex&&a.pass2Index===b.pass2Index;
  };
  SE.saveSearchIndexCache=async function(reason=''){
    try{
      const S=window.SearchEngine, app=window.App;
      if(!S||!app||!Array.isArray(app.assets)||!app.assets.length){this.indexCacheStatus='empty';return false;}
      if(S.indexRunning){this.indexCacheStatus='busy';return false;}
      const sig=this.searchIndexSignature();
      const assets=app.assets;
      const idByAsset=new Map(), assetIds=[];
      for(let i=0;i<assets.length;i++){
        const a=assets[i]; if(!a||typeof a!=='object')continue;
        const id=this._assetIdForCache(a,i); if(!id)continue;
        if(!idByAsset.has(a)){idByAsset.set(a,id); assetIds.push(id);}
      }
      const idsFor=(list=[])=>{
        const out=[];
        for(const a of (Array.isArray(list)?list:[])){
          const id=idByAsset.get(a)||String(a?.id||'');
          if(id)out.push(id);
        }
        return out;
      };
      const lineMap=[];
      try{
        for(const [key,g] of (S.lineMap||new Map())){
          lineMap.push([key,{line:g?.line||'',rawLine:g?.rawLine||'',validGps:Number(g?.validGps||0),assetIds:idsFor(g?.assets),routeIds:idsFor(g?.routeAssets)}]);
        }
      }catch(_e){}
      const spatialIndex=[];
      try{for(const [key,list] of (S.spatialIndex||new Map()))spatialIndex.push([key,idsFor(list)]);}catch(_e){}
      const kindIndex={};
      try{for(const [key,list] of Object.entries(S.kindIndex||{}))kindIndex[key]=idsFor(list);}catch(_e){}
      const circuitPathIndex=[];
      try{for(const [key,g] of (S.circuitPathIndex||new Map()))circuitPathIndex.push([key,g]);}catch(_e){}
      const circuitEndpointPathIndex=[];
      try{for(const [key,set] of (S.circuitEndpointPathIndex||new Map()))circuitEndpointPathIndex.push([key,Array.from(set||[])]);}catch(_e){}
      const referencePointsByCode=[];
      try{for(const [key,list] of (S.referencePointsByCode||new Map()))referencePointsByCode.push([key,idsFor(list)]);}catch(_e){}
      const referencePoints=idsFor(S.referencePoints||[]);
      const cache={
        savedAt:new Date().toISOString(),reason:String(reason||''),sig,assetIds,
        lineMap,spatialIndex,kindIndex,circuitPathIndex,circuitEndpointPathIndex,referencePointsByCode,referencePoints,
        circuitPathStats:S.circuitPathStats||null,indexStats:{...(S.indexStats||{}),savedLightStartupCache:true,lazyTextSearch:true}
      };
      await this.set(this.searchIndexCacheKey,cache);
      app.indexHealth=app.indexHealth||{mode:'file-level',queue:[],files:[]};
      app.indexHealth.startupIndex={status:'saved',savedAt:cache.savedAt,assetCount:sig.assetCount,fileCount:sig.fileCount,reason:String(reason||''),mode:'light startup cache'};
      await this.set('indexHealth',app.indexHealth).catch(()=>{});
      this.indexCacheStatus='saved'; this.indexCacheMessage=`Saved startup index for ${sig.assetCount.toLocaleString()} records`;
      return true;
    }catch(err){
      this.indexCacheStatus='save failed'; this.indexCacheMessage=String(err?.message||err);
      try{window.Diagnostics?.log?.('Startup index cache save failed',this.indexCacheMessage);}catch(_e){}
      return false;
    }
  };
  SE.loadSearchIndexCache=async function(){
    try{
      const S=window.SearchEngine, app=window.App;
      if(!S||!app||!Array.isArray(app.assets)||!app.assets.length){this.indexCacheStatus='empty';return false;}
      const cache=await this.get(this.searchIndexCacheKey);
      const cur=this.searchIndexSignature();
      if(!cache||!cache.sig){this.indexCacheStatus='missing';this.indexCacheMessage='No saved startup index yet';return false;}
      if(!this.sameSearchIndexSignature(cache.sig,cur)){
        this.indexCacheStatus='stale';this.indexCacheMessage='Saved startup index does not match current imported files';return false;
      }
      const ok=await S.restoreLightIndexCache?.(cache);
      if(ok){
        this.indexCacheStatus='loaded';this.indexCacheMessage=`Loaded saved startup index from ${cache.savedAt||'previous session'}`;
        app.indexHealth=app.indexHealth||{mode:'file-level',queue:[],files:[]};
        app.indexHealth.startupIndex={status:'loaded',savedAt:cache.savedAt,loadedAt:new Date().toISOString(),assetCount:cur.assetCount,fileCount:cur.fileCount,mode:'light startup cache'};
        await this.set('indexHealth',app.indexHealth).catch(()=>{});
        return true;
      }
      this.indexCacheStatus='restore failed';this.indexCacheMessage='Saved startup index could not be restored';return false;
    }catch(err){
      this.indexCacheStatus='load failed';this.indexCacheMessage=String(err?.message||err);
      try{window.Diagnostics?.log?.('Startup index cache load failed',this.indexCacheMessage);}catch(_e){}
      return false;
    }
  };
  SE.clearSearchIndexCache=async function(){
    await this.del(this.searchIndexCacheKey).catch(()=>{});
    this.indexCacheStatus='cleared'; this.indexCacheMessage='Startup index cache cleared';
  };
})();
