...
/* myMap v3.1.239: direct asset search crossing bay marker fix.
   When a single searched pole/tower is opened, draw the HV/TX markers that belong
   to the same crossing bay.  The previous path could list matched crossings but
   miss drawing them because it only accepted exact from/to text matches. */
(function(){
  const H=typeof window!=='undefined'?window.HVCrossingsLayer:null;
  if(!H||H.__v239AssetBayCrossingMarkers)return; H.__v239AssetBayCrossingMarkers=true;
  const S=()=>window.SearchEngine||{};
  const M=()=>window.MapEngine||{};
  const clean=(v)=>String(v==null?'':v).trim();
  const compact=(v)=>{try{return S().compact?S().compact(v):clean(v).toUpperCase().replace(/[^A-Z0-9]+/g,'');}catch(_){return clean(v).toUpperCase().replace(/[^A-Z0-9]+/g,'');}};
  const fmtLine=(v)=>{try{return S().formatCircuitName?S().formatCircuitName(v):clean(v).replace(/\s+/g,' ').toUpperCase();}catch(_){return clean(v).replace(/\s+/g,' ').toUpperCase();}};
  const valid=(lat,lon)=>Number.isFinite(Number(lat))&&Number.isFinite(Number(lon))&&Math.abs(Number(lat))<=90&&Math.abs(Number(lon))<=180;
  const point=(a)=>{if(!a)return null; const lat=Number(a.lat??a.latitude??a.Latitude??a.LATITUDE); const lon=Number(a.lon??a.lng??a.longitude??a.Longitude??a.LONGITUDE); return valid(lat,lon)?{lat,lon}:null;};
  const distKm=(a,b)=>{if(!a||!b)return Infinity; const lat1=Number(a.lat),lon1=Number(a.lon),lat2=Number(b.lat),lon2=Number(b.lon); if(!valid(lat1,lon1)||!valid(lat2,lon2))return Infinity; const dy=(lat1-lat2)*110.574; const dx=(lon1-lon2)*111.320*Math.max(.18,Math.cos(((lat1+lat2)/2)*Math.PI/180)); return Math.hypot(dx,dy);};
  const raw=(a)=>a?.raw||{};
  function polePartsFromText(v){
    const t=clean(v).toUpperCase(); if(!t)return [];
    const out=[]; const seen=new Set();
    const push=(num,suf='')=>{num=Number(num); suf=clean(suf).toUpperCase(); if(!Number.isFinite(num))return; const key=num+'|'+suf; if(seen.has(key))return; seen.add(key); out.push({num,suf,norm:String(num)+suf});};
    try{const p=S().poleIdParts?.(t); if(p)push(p.num,p.suffix||p.suf||'');}catch(_){ }
    for(const m of t.matchAll(/\b[A-Z]{0,4}0*(\d{1,6})([A-Z]{0,3})\b/g))push(m[1],m[2]||'');
    for(const m of t.matchAll(/(?:^|[\s\-\/])0*(\d{1,6})([A-Z]{0,3})(?=$|[\s\-\/])/g))push(m[1],m[2]||'');
    return out;
  }
  function assetPoleParts(asset){
    const r=raw(asset); const vals=[asset?.poleNumber,asset?.structure,asset?.label,asset?.gisLabel,asset?.rawStructure,r.NAMEPLATE_ID_1,r.NAMEPLATE_ID,r.FIRST_NAME_PLATE_ID,r.LAST_NAME_PLATE_ID,r.STRUCTURE_ID,r.STRUCTURE_LABEL,r.POLE_NUMBER,r.POLE_NO,r.TRMSN_LINE_GIS_LABEL,r.trmsn_line_gis_label];
    try{for(const ref of S().lineRefsForAsset?.(asset,true)||[])vals.push(ref?.pole,ref?.label,ref?.structure);}catch(_){ }
    const out=[]; const seen=new Set();
    for(const v of vals){for(const p of polePartsFromText(v)){const k=p.num+'|'+p.suf; if(!seen.has(k)){seen.add(k);out.push(p);}}}
    return out;
  }
  function crossingPoleParts(rec){
    const r=raw(rec); const vals=[rec?.from,rec?.to,rec?.title,rec?.label,r.from_label,r.to_label,r.from_pole_no,r.to_pole_no,r.nearest_tx_structure,r.nearest_tx_structure_1,r.nearest_tx_structure_2,r.from,r.to,r.title,r.name,r.label];
    const out=[]; const seen=new Set();
    for(const v of vals){for(const p of polePartsFromText(v)){const k=p.num+'|'+p.suf; if(!seen.has(k)){seen.add(k);out.push(p);}}}
    return out;
  }
  function directPoleTextTouch(rec,asset){
    const aParts=assetPoleParts(asset); if(!aParts.length)return false;
    const cParts=crossingPoleParts(rec); if(!cParts.length)return false;
    for(const a of aParts){
      for(const c of cParts){
        // Exact same structure number, or the immediately adjacent bay endpoint.
        if(a.num===c.num&&(!a.suf||!c.suf||a.suf===c.suf))return true;
      }
    }
    return false;
  }
  function spatialBayTouch(rec,asset){
    const ap=point(asset), rp=point(rec); if(!ap||!rp)return false;
    const d=distKm(ap,rp);
    if(d<=0.75)return true; // covers normal transmission bay spans around the selected pole/tower.
    return false;
  }
  function lineRefs(asset){
    const out=[]; const seen=new Set();
    const add=(v)=>{const f=fmtLine(v); const k=compact(f); if(f&&k&&!seen.has(k)&&/(?:-|\/)/.test(f)){seen.add(k);out.push(f);}};
    try{for(const r of H.lineRefsForBayAsset?.(asset)||[])add(r?.line);}catch(_){ }
    try{for(const r of S().lineRefsForAsset?.(asset,true)||[])add(r?.line);}catch(_){ }
    const r=raw(asset);
    [asset?.line,r.LINE_NAME,r.LINE_NAME_1,r.LINE_NAME_2,r.CIRCUIT,r.CIRCUIT_NAME,r.TRMSN_LINE_GIS_LABEL,r.trmsn_line_gis_label,asset?.gisLabel,asset?.label].forEach(add);
    return out;
  }
  function layerAllows(rec){
    try{const key=String(rec?.type||'').toUpperCase()==='TX'?'crossTx':'crossDx'; return M().assetLayerEnabled?M().assetLayerEnabled(key):true;}catch(_){return true;}
  }
  const oldTouch=H.crossingTouchesAssetBay;
  H.crossingTouchesAssetBay=function(rec,asset){
    try{if(oldTouch&&oldTouch.call(this,rec,asset))return true;}catch(_){ }
    try{if(directPoleTextTouch(rec,asset))return true;}catch(_){ }
    return false;
  };
  const oldShowBay=H.showBayForAsset;
  H.showBayForAsset=async function(asset,opts={}){
    await this.loadStore?.(); this.init?.();
    if(!asset){this.draw?.([],'asset-bay',''); return 0;}
    const lines=lineRefs(asset);
    if(!lines.length){
      const c=oldShowBay?await oldShowBay.call(this,asset,Object.assign({},opts,{silent:true})):0;
      if(c&&!opts.silent)try{window.UI?.toast?.(`${c} crossing marker(s) shown for this asset bay.`);}catch(_){ }
      return c||0;
    }
    const out=[];
    for(const line of lines){
      let base=[]; try{base=this.matchesForLine?.(line)||[];}catch(_){base=[];}
      for(const rec of base){
        if(!layerAllows(rec))continue;
        let hit=false;
        try{hit=this.crossingTouchesAssetBay?.(rec,asset)||spatialBayTouch(rec,asset);}catch(_){hit=false;}
        if(hit)out.push(rec);
      }
    }
    let list=[];
    try{list=this.dedupeList?this.dedupeList(out):out;}catch(_){list=out;}
    // Keep the asset-search view clean. Show the closest few markers if there are several in the area.
    const ap=point(asset);
    if(ap&&list.length>8){list=list.map(r=>Object.assign({_bayKm:distKm(ap,point(r))},r)).sort((a,b)=>(a._bayKm||999)-(b._bayKm||999)).slice(0,8);}
    const count=this.draw?.(list,'asset-bay',lines.join(' + '))||0;
    if(count&&!opts.silent){
      const dx=list.filter(r=>String(r?.type).toUpperCase()!=='TX').length, tx=list.filter(r=>String(r?.type).toUpperCase()==='TX').length;
      try{window.UI?.toast?.(`${count} crossing marker(s) shown for this asset bay: ${dx} HV, ${tx} TX.`);}catch(_){ }
    }
    return count;
  };
})();
